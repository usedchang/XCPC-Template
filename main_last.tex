...
\usepackage{geometry}
\geometry{a4paper, margin=1.3cm}  % 调整页边距以适应双栏
...
\subsection{编译指令}
\begin{lstlisting}[caption={编译指令}]
g++ -std=c++20 -Wall -Wextra -O2 a.cpp -o a.exe && a.exe//编译程序并运行
/*
-Wall：开启大部分常用警告
-Wextra：开启额外警告，帮助发现潜在问题
-O2：二级优化，平衡编译速度与运行效率，竞赛中最常用
-o a.exe：指定输出文件名（在 Linux 下可改为 -o a）
*/
\end{lstlisting}

\begin{lstlisting}[caption={.bat/.cmd (win)批处理文件编译}]
@echo off
g++ -std=c++17 -O2 -Wall -o sol.exe sol.cpp && sol.exe
pause
\end{lstlisting}

\begin{lstlisting}[caption={.sh (linux)文件编译}]
#!/bin/bash
g++ -std=c++17 -O2 -Wall -o sol sol.cpp && ./sol
\end{lstlisting}

\subsection{cpp扩栈(win)}
\begin{lstlisting}[caption={扩栈指令}]
int size(256<<20);
...
\begin{lstlisting}[caption={可撤销并查集}]
struct DSU {
    vector<int> f, siz;
    struct Record {
        int smallRoot;
...
\begin{lstlisting}[caption={FHQ-Treap},label={lst:FHQ-Treap}]
template <typename T>
struct FHQ_Treap {
    struct Node {
        T data;
...
\end{itemize}
\begin{lstlisting}[caption={树上启发式合并}]
//U41492 树上数颜色
#include<bits/stdc++.h>
using namespace std;
#define endl '\n'
...
\subsection{KMP}
\begin{lstlisting}[caption={KMP1},label={KMP1}]
...
\begin{lstlisting}[caption={KMP2},label={KMP2}]
...
\subsection{FFT}
\begin{lstlisting}[caption={FFT解决区间和是质数的数的区间个数}]
#include <bits/stdc++.h>
using namespace std;
typedef long long ll;
#define endl '\n'
...
\begin{lstlisting}[caption={Pollard-Rho}]
#include <bits/stdc++.h>
using namespace std;
using ll = long long;
using i128 = __int128;
...
\subsection{异或线性基}
\begin{lstlisting}[caption={异或线性基},label={异或线性基}]
struct LB{
    vector<ll>a;
    bool flag;
    LB(){
...
\section{计算几何}
\subsection{极角排序}
\begin{lstlisting}[caption={极角排序求夹角最近两向量}]
#include<bits/stdc++.h>
using namespace std;
#define endl '\n'
typedef long long ll;
